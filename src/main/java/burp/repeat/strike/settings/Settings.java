package burp.repeat.strike.settings;
...
import static burp.repeat.strike.utils.GridbagUtils.addMarginToGbc;
...
public class Settings {
    ...
    public static void showSettingsWindow() {
        Settings settings = new Settings("general", RepeatStrikeExtension.callbacks);
        burp.repeat.strike.utils.Utils.registerGeneralSettings(settings);
        settings.load();
        ...
        try {
            ...
            RepeatStrikeExtension.callbacks.printError("Error building interface:" + e);
    ...
    private JSONObject getSettingsObject(String name) {
        ...
        while(keys.hasNext()) {
            String key = keys.next();
            JSONObject setting = this.settings.getJSONObject(key);
            if(!setting.has("value")) {
    ...
    public JPanel buildInterface(JFrame settingsWindow, int componentWidth, int componentHeight, int spacing, Map<String, Integer> columns, Settings loadSettingObject) throws UnregisteredSettingException, InvalidTypeSettingException {
                    ...
                    case "Boolean" -> {
                        ...
                        checkBox.addActionListener(e -> {
                            ...
                            if(isChecked && shouldWarn) {
                                if(confirm(checkBox, "Confirm", currentSetting.getString("warning"))) {
        ...
        resetSettingsBtn.addActionListener(e -> {
            if(confirm(null, "Confirm reset setting", "Are you sure you want to reset your settings?")) {
                this.resetSettings();
                this.save();
        ...
        closeSettingsBtn.addActionListener(e -> {
            if(isModified) {
                if(confirm(null, "Confirm", "Are you sure you have unsaved settings?")) {
                    return;
                }
            }
